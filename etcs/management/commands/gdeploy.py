...
def deploy_media(conf):
    ...
    if os.path.exists(os.path.join(MEDIA_ROOT,"static")):
        _("rm %s" % (os.path.join(MEDIA_ROOT,"static")))
    _("ln -s %s %s" % (os.path.join(PROJECT_ROOT,"media","static"), os.path.join(MEDIA_ROOT,"static")))
